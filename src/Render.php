...
function renderPlane(array $data): string
{
    ...
    $arr = array_map(function ($item) use ($sign) {
        if ($item['status'] === 'modifed') {
            $item['valueBefore'] = is_bool($item['valueBefore']) ? var_export($item['valueBefore'], true) : $item['valueBefore'];
            $item['valueAfter'] = is_bool($item['valueAfter']) ? var_export($item['valueAfter'], true) : $item['valueAfter'];
